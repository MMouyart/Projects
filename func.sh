...
apikey=$(vtapikey)
vurl() {
response=$(curl -s --request POST --url https://www.virustotal.com/api/v3/urls --header 'x-apikey: $apikey' --form url=$1 | jq -r '.data.id')
parsed_part="${response#*-}"
parsed_part="${parsed_part%%-*}"
url="https://www.virustotal.com/api/v3/urls/$parsed_part"
response=$(curl -s --request GET --url $url --header 'x-apikey: $apikey' | grep category | cut -d ':' -f 2 | sort | uniq -c)
echo -e $response}
...
--url $url \
--header 'x-apikey: apikey' | grep category | cut -d ':' -f 2 | sort | uniq -c);
echo $response; 

elif [[ $size -gt 20000000 && $size -le 200000000 ]]; then
url=$(curl -s --request GET \
--url https://www.virustotal.com/api/v3/files/upload_url \
--header 'x-apikey: $apikey' | jq -r '.data')
echo "uploading file to virustotal"
id=$(curl -s --request POST \
	--url $url \
	--header 'accept: application/json' \
...
response=$(curl -s --request GET \
--url $url \
--header 'x-apikey: $apikey' | jq -r '.data.attributes.stats');
echo $response;
...
response=$(curl -s --request GET \
	...
	--header 'x-apikey: $apikey' | jq -r '.data.attributes.last_analysis_stats') ;
echo $response;}

vdomain() {
response=$(curl -s --request GET \
	--url https://www.virustotal.com/api/v3/domains/$1 \
	--header 'x-apikey: $apikey' | jq -r '.data.attributes.last_analysis_stats') ;
echo $response;}
...
function docBackup(){
	split -b 4294967295 docBackup.tar.gz "docBackup.tar.gz.part"
	for i in *.gz.part*; do
		echo "moving file $i to $1"
		mv $i $1
	done
}
