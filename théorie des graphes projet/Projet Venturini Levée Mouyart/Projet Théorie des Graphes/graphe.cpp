...
{
    std::ofstream ancien ("ancien.txt");

    for(int i = 0; i<m_ordre; i++)
    {
        ancien<<m_sommets[i]->getDegre() <<std::endl;
         ancien<<m_sommets[i]->getDegreN() <<std::endl;
         ancien<<m_sommets[i]->getVectPropre() <<std::endl;
         ancien<<m_sommets[i]->getCentralProx() <<std::endl;
         ancien<< m_sommets[i]->getIntermediarite() <<std::endl;
...
void Graphe::indiceChargement()
{
    std::ifstream ancien ("ancien.txt");
    int deg;
    float degN, inte, prox, vect;
    for(int i = 0; i<m_ordre; i++)
    {
        ancien>>deg;
         ancien>>degN;
         ...
         ancien>>prox;
         ancien>>vect;
        m_sommets[i]->setAncienCentralProx(prox);
        m_sommets[i]->setAncienDegre(deg);
        m_sommets[i]->setAncienDegreN(degN);
        m_sommets[i]->setAncienIntermediarite(inte);
        m_sommets[i]->setAncienVectPropre(vect);
    ...
    for(int k=0; k<m_ordre;k++)
    {
        ...
        else ///Sinon
        {
            ...
            std::cout<<"        " << m_sommets[k]->getCentralProx() << "      "<< m_sommets[k]->getCentralProx() * (m_ordre-1);
             std::cout<<"         "<< m_sommets[k]->getIntermediarite()<<"      "<< m_sommets[k]->getIntermediarite()/(m_ordre * m_ordre - 3*m_ordre + 2);
 ...
 void Graphe::afficherAncien()
 {
    ...
    for(int k=0; k<m_ordre;k++)
    {
        ...
        else ///Sinon
        {
            ...
            std::cout<< "      "<< m_sommets[k]->getAncienVectPropre() ;
            std::cout<<"        " << m_sommets[k]->getAncienCentralProx() << "      "<< m_sommets[k]->getAncienCentralProx() * (m_ordre-1);
             std::cout<<"         "<< m_sommets[k]->getAncienIntermediarite()<<"      "<< m_sommets[k]->getAncienIntermediarite()/(m_ordre * m_ordre - 3*m_ordre + 2);
         std::cout<<std::endl;
         std::cout<<std::endl;
        }
    }
